...
public class MainMenu10_4_3_Login_Re_Establecer_Clave extends ToolsAppCompact implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv1.setText("Ingresa datos de administrador");
        InitTrans.wrlg.wrDataTxt("Llamado al método temporizador desde onCreate, Login re-establecer clave.");
        temporizador();
        btnLogin.setTypeface(tipoFuente2);
            ...
            public void afterTextChanged(Editable s) {
                if(et_user.length() == 10){
                    ...
                    if(PERSON != null){
                        ...
                        InitTrans.wrlg.wrDataTxt("Reinicio timer Login re-establecer clave, usuario no existe.");
                        reiniciarTimer();
                        et_user.setText("");
                        et_user.setBackgroundResource(R.drawable.edittext_pichi_error);
    ...
    public void onBackPressed() {
        InitTrans.wrlg.wrDataTxt("Fin timer Login re-establecer clave, desde botón back, ingresa a usuarios locales.");
        countDownTimer.cancel();
        UIUtils.startView(MainMenu10_4_3_Login_Re_Establecer_Clave.this, MainMenu10_4_UsuariosLocales.class);
    }
        ...
        if (!user.isEmpty() || !user.isEmpty()) {
            if (Login.persona.getUser().equals(user)) {
                ...
                if (PERSON.getRole().equals("ADMINISTRADOR") && PERSON.getPassword().equals(pass)) {
                    ...
                    InitTrans.wrlg.wrDataTxt("Fin timer Login re-establecer clave, ingresa a reestablecer clave.");
                    countDownTimer.cancel();
                    UIUtils.startView(this,MainMenu10_4_3_1_Re_Establecer_Clave.class);
                } else {
                    if (intento >= 3) {
                        ...
                        reiniciarTimer();
                        intento++;
                        InitTrans.wrlg.wrDataTxt("Reinicio timer Login re-establecer clave, contraseña incorrecta " + intento);
                        valor=Integer.toString(intento);
                        conexion10.updateFields(user,"user_intento",valor,"user");
                        et_pw.setText("");
                ...
                InitTrans.wrlg.wrDataTxt("Reinicio timer Login re-establecer clave, debe ingresar el usuario actual.");
                reiniciarTimer();
                et_user.setText("");
                et_pw.setText("");
                et_user.setBackgroundResource(R.drawable.edittext_pichi_error);
            ...
            InitTrans.wrlg.wrDataTxt("Reinicio timer Login re-establecer clave, usuario y clave no pueden estar vacíos.");
            reiniciarTimer();
            et_user.setBackgroundResource(R.drawable.edittext_pichi_error);
    ...
    public void temporizador(){
        InitTrans.wrlg.wrDataTxt("Inicio timer Login re-establecer clave");
        countDownTimer = new CountDownTimer(InitTrans.time, 1000) {
            public void onTick(long millisUntilFinished) {
            }
            public void onFinish() {
                InitTrans.wrlg.wrDataTxt("Fin timer Login re-establecer clave, desde método temporizador, ingresa a usuarios locales");
                countDownTimer.cancel();
                UIUtils.startView(MainMenu10_4_3_Login_Re_Establecer_Clave.this, MainMenu10_4_UsuariosLocales.class);
            }
        }.start();
    ...
    protected void onPause() {
        super.onPause();
        InitTrans.wrlg.wrDataTxt("Fin timer Login re-establecer clave, desde onPause.");
        countDownTimer.cancel();
    }
    ...
    protected void onResume() {
        super.onResume();
        InitTrans.wrlg.wrDataTxt("Reinicio timer Login re-establecer clave, desde onResume.");
        reiniciarTimer();
    }
