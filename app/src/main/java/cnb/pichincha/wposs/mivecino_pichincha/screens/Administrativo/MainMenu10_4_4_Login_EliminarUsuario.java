...
public class MainMenu10_4_4_Login_EliminarUsuario extends ToolsAppCompact implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        InitTrans.wrlg.wrDataTxt("Llamado al método temporizador desde onCreate, Login eliminar usuario.");
        temporizador();
            ...
            public void afterTextChanged(Editable s) {
                if(et_user.length() == 10){
                    ...
                    if(PERSON != null){
                        ...
                        InitTrans.wrlg.wrDataTxt("Reinicio timer Login eliminar usuario, usuario no existe.");
                        reiniciarTimer();
                        et_user.setText("");
                        et_user.setBackgroundResource(R.drawable.edittext_pichi_error);
        ...
        if (!user.isEmpty() || !pass.isEmpty()) {
            if (Login.persona.getUser().equals(user)) {
                ...
                if (PERSON.getRole().equals("ADMINISTRADOR") && PERSON.getPassword().equals(pass)) {
                    ...
                    InitTrans.wrlg.wrDataTxt("Fin timer Login eliminar usuario, ingresa a eliminar usuario.");
                    countDownTimer.cancel();
                    UIUtils.startView(this,MainMenu10_4_4_1_EliminarUsuario.class);
                } else {
                    if (intento >= 3) {
                        ...
                        reiniciarTimer();
                        intento++;
                        InitTrans.wrlg.wrDataTxt("Reinicio timer Login eliminar usuario, contraseña incorrecta " + intento);
                        valor=Integer.toString(intento);
                        conexion10.updateFields(user,"user_intento",valor,"user");
                        et_pw.setText("");
                ...
                InitTrans.wrlg.wrDataTxt("Reinicio timer Login eliminar usuario, debe ingresar el usuario actual.");
                reiniciarTimer();
                et_user.setText("");
                et_pw.setText("");
                et_user.setBackgroundResource(R.drawable.edittext_pichi_error);
            ...
            InitTrans.wrlg.wrDataTxt("Reinicio timer Login eliminar usuario, usuario y clave no pueden estar vacíos.");
            reiniciarTimer();
            et_user.setBackgroundResource(R.drawable.edittext_pichi_error);
    ...
    public void onBackPressed() {
        InitTrans.wrlg.wrDataTxt("Fin timer Login eliminar usuario, desde botón back, ingresa a usuarios locales.");
        countDownTimer.cancel();
        UIUtils.startView(MainMenu10_4_4_Login_EliminarUsuario.this, MainMenu10_4_UsuariosLocales.class);
    }
    ...
    public void temporizador(){
        InitTrans.wrlg.wrDataTxt("Inicio timer Login eliminar usuario");
        countDownTimer = new CountDownTimer(InitTrans.time, 1000) {
            public void onTick(long millisUntilFinished) {
            }
            public void onFinish() {
                InitTrans.wrlg.wrDataTxt("Fin timer Login eliminar usuario, desde método temporizador, ingresa a usuarios locales.");
                countDownTimer.cancel();
                UIUtils.startView(MainMenu10_4_4_Login_EliminarUsuario.this, MainMenu10_4_UsuariosLocales.class);
            }
        }.start();
    ...
    protected void onPause() {
        super.onPause();
        InitTrans.wrlg.wrDataTxt("Fin timer Login eliminar usuario, desde onPause.");
        countDownTimer.cancel();
    }
    ...
    protected void onResume() {
        super.onResume();
        InitTrans.wrlg.wrDataTxt("Reinicio timer Login eliminar usuario, desde onResume.");
        reiniciarTimer();
    }
