...
public class ISOUtil {
	...
	public static String padright(String s, int len, char c) {
		...
		while (fill-- > 0)
			d.append(c);
		//Log.e("padright",d.toString());
		return d.toString();
	}

	/**
	 * cut the string when to find the character
	 *
	 * @param s
	 *            - original string
	 * @param c
	 ...
	 * @return padded string
	 */
	public static String cutChar(String s, String c) {
		String [] aux;
		aux=s.split(c);
		s=aux[0];
		return s;
	 ...
	 * @return
	 */
	public static String formatMiles( int valor){
		String val = String.valueOf(valor);
        String elnumero = val;
		if(val.length() >2){
            elnumero = val.substring(0, val.length() -2)+"."+val.substring(val.length() -2);
        }else{
			elnumero = "00."+val;
		}
		DecimalFormat formatMinimoRetiro = new DecimalFormat("###,##0.00");
		return (formatMinimoRetiro.format(Double.parseDouble(elnumero)));
	}
	...
	public static void grabar(byte[] bs) {
        /*String nomarchivo = "";
		try {
			nomarchivo = PAYUtils.getLocalDate() + " " + PAYUtils.getLocalTime() + " ";
			nomarchivo += Trans.MsgID + " " + Trans.ProcCode;
			nomarchivo += ".txt";
			String contenido = byte2hex(bs, 0, bs.length);
			try {
				String carpeta = "/sdcard/logs/ISO";
				File dir = new File(carpeta);
				if (!dir.exists()) {
					dir.mkdirs();
				}
				//File tarjeta = Environment.getExternalStorageDirectory();
				File file = new File(carpeta + "/" + nomarchivo);
                ...
                InitTrans.wrlg.wrDataTxt("Error en proceso de guardado - " + nomarchivo );
			}
		} catch (Exception e){
            InitTrans.wrlg.wrDataTxt("Error en guardarTxt() - " + nomarchivo);
		}*/
	}

	public static void guardarTxt(String data, String direc, String nameFile){
		/*try {
            ...
            InitTrans.wrlg.wrDataTxt("Error en guardarTxt() - " + nameFile);
		}*/
	}
	...
	private static String hexOffset(int i) {
		i = i >> 4 << 4;
		int w = i > 0xFFFF ? 8 : 4;
		return padleft(Integer.toString(i, 16), w,'0');
	}
	...
	public static boolean isNumerico(String data) {
		boolean ret;
