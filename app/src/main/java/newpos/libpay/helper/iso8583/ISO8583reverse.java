 ...
 * @author
 */
public class ISO8583reverse {

	// The zeroth field is the message type
	// The first domain is BITMAP

	private boolean isHasMac = false;
	private int fieldNum; 				// Length of the field
	private FieldAttr[] listFieldAttr; 	// Description of the content format of the domain
	private String[] listFieldData; 	// Data collection for the domain (data for each domain)

	private String[] listFieldRespData;	// Return the domain data of the data
	...
	public interface RSPCODE{
		...
		public static final String RSP_BATCH_LOAD = "94";
		public static final String RSP_NEW_USER_XPERTA = "95";
		public static final String RSP_MULTICUENTA = "97";
		public static final String RSP_TELECARGA = "98";
	}
	 ...
	 */
	public ISO8583reverse(Context context, String tpdu, String header) {
		...
		listFieldData = new String[fieldNum + 1];//0~64 has 65
	...
	public byte[] packetISO8583() {
		...
		for (int i = 2; i < fieldNum; i++) {
			...
			if (dataType == FieldTypesDefine.FIELDATTR_TYPE_BIN) {
				...
				temp = ISOUtil.str2bcd(fieldData, false, (byte) 0);
				dataLen = temp.length;
				System.arraycopy(temp, 0, bb, offset, dataLen);
			} else if (dataType == FieldTypesDefine.FIELDATTR_TYPE_N) {
				temp = ISOUtil.str2bcd(fieldData, true, (byte) 0);
				dataLen = temp.length;
				System.arraycopy(temp, 0, bb, offset, dataLen);
			}
			offset += dataLen;
